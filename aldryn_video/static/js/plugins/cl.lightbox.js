	...
	Cl.Lightbox = new Class({
		...
		options: {
			...
			'lang': {
				...
				'next': 'Weiter',
				'previous': 'Zurück',
				'status': 'Aufgabe {current} von {total}.'
			}
		},
		...
		initialize: function (triggers, options) {
			...
			// set the collection
			this.collection = this.options.collection || $(triggers).data('collection');
			...
			this.triggers.on('click', function (e) {
				e.preventDefault();
				if (that.collection) {
					that.open.call(that, that.collection[0]);
				} else {
		...
		_preload: function (el) {
			...
			if(this.options.group && this.triggers.length > 1) {
				...
				this.index = this.collection.indexOf(this.url);
				this.bound = this.collection.length;
			}
